...
public class MARCRecord extends DataObject {
  ...
  public static final  char RECORD_DELIMITER   = (char)0x1d;
  public static final  char FIELD_DELIMITER    = (char)0x1e;
  public static final  char SUBFIELD_DELIMITER = (char)0x1f;
  private static final char MARC8_ENCODING     = (char)0x20;
...
/** @todo handle exception */
  ...
  private static void write(ByteArrayOutputStream byteArray, int length, int value) {
      int card = (int)Math.floor(Math.log10(value)) + 1;
      byte [] bytes = new byte[length];
      Arrays.fill(bytes, (byte)0x30);
      if (value > 0 && length >= card) {
          for (int i = length - card; i < length; i++){
              int exp = length - (i+1); // exponent from position
              int digit = (int)(value / Math.pow(10, exp));
              bytes[i] = (byte)DIGITS[digit];
              value = (int)(value % Math.pow(10, exp));
          }
      }
      try {byteArray.write(bytes); } catch (IOException e){}
